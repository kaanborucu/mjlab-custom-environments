...
def base_lin_vel_heading(
  ...
  """Root linear velocity in a yaw-aligned frame with world-aligned vertical."""
  asset: Entity = env.scene[asset_cfg.name]
  heading_quat_w = yaw_quat(asset.data.root_link_quat_w)
  return quat_apply_inverse(heading_quat_w, asset.data.root_link_lin_vel_w)


def base_lin_vel_body_xy_world_z(
  env: ManagerBasedRlEnv, asset_cfg: SceneEntityCfg = _DEFAULT_ASSET_CFG
) -> torch.Tensor:
  """Root body-frame horizontal velocity with world-frame vertical velocity."""
  asset: Entity = env.scene[asset_cfg.name]
  return torch.cat(
    (
      asset.data.root_link_lin_vel_b[:, :2],
      asset.data.root_link_lin_vel_w[:, 2:3],
    ),
    dim=1,
  )
